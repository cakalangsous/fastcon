        ...
        foreach($fastcon_banners as $fastcon_banner){
			        	...
			        	if($this->is_allowed('fastcon_banner_update'))
        	{
		        $button .= '<a href="'.site_url("administrator/fastcon_banner/edit/" . $fastcon_banner->id) .'" class="label-default mr-3"><i class="fa fa-edit "></i> '.cclang('update_button').'</a>';
			        	...
			        	if($this->is_allowed('fastcon_banner_delete'))
        	{
		        $button .= '<a href="javascript:void(0);" onclick="delete_this(\''.BASE_URL.'administrator/fastcon_banner/delete/'.$fastcon_banner->id.'\')" class="label-default remove-data"><i class="fa fa-close"></i>'.cclang('remove_button').'</a>';
        	}
