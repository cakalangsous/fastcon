...

	{
		...
		if ($this->form_validation->run()) {
			...
			if ($this->input->post('create')) {
				...
				$this->aauth->create_perm($table_name.'_add','',$table_name);
			...
			if ($this->input->post('update')) {
				...
				$this->aauth->create_perm($table_name.'_update','',$table_name);
			...
			if ($this->input->post('read')) {
				...
				$this->aauth->create_perm($table_name.'_view','',$table_name);

			}



			$this->aauth->create_perm($table_name.'_delete','',$table_name);

			$this->aauth->create_perm($table_name.'_list','',$table_name);
			
			$this->aauth->create_perm('menu_'.$table_name,'',$table_name);
			...
			//20180503 added for insert to menu table when adding crud by Benkin
			$this->load->model('model_menu');

			$menu_exist = $this->model_menu->get_menu_by_lable($this->input->post('title'),'administrator/'.$table_name);// check if menu exist 20180810 by Benkin

			if (!$menu_exist) { //20180810 by Benkin
				$save_data_menu = [

					'label' 		=> $this->input->post('title'),

					'link' 			=> 'administrator/'.$table_name,
				...
				$this->db->insert('menu', $save_data_menu); // 20180503 save to menu table by Benkin
			}
	...
	*/
...

	{
		...
		if ($crud->table_name) {
			...
			//delete menu if crud deleted 20180504 by Benkin
			$this->remove_menu($table_name);
			...
			$this->db->where_in('name', [
				...
				'menu_'.$table_name //delete menu perms also 20180504 by Benkin
				]
	...
	public function remove_menu($table_name='')
	{
		$temp = str_replace('_', ' ', $table_name);
		$menu_name = _ent(ucwords(clean_snake_case($temp)));
		
		$id = $this->db->where('label',$menu_name);
		$query = $this->db->get('menu')->result();

		$id_menu = 0;
		if ($query) {
			$this->db->where('id',$query[0]->id);
			$del = $this->db->delete('menu');
		}
	}
