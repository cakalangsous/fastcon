...
class Group extends Admin	
{
	...
	*/
	public function add_save()
	{
		...
		if ($this->form_validation->run()) {
			...
			$get_last_group = $this->db->get_where('aauth_groups',$save_data)->result();
			$last_id = 0;
			if ($get_last_group) {
				$last_id = $get_last_group[0]->id;
			}
			$permission = array(1,20,21,22);
	...
	*/
	private function _remove($id)
	{
		...
		$perm = $this->db->get_where('aauth_perm_to_group',$group_id)->result();
		if ($perm) {
			$this->db->delete('aauth_perm_to_group',$group_id);
		}
