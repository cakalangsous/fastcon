...
if(!function_exists('db_get_all_data')) {
	...
	function db_get_all_data($table_name = null, $where = false, $limit=false, $from=false, $select_field=false, $order_by=false, $group_by=false) {
		...
		if ($order_by) {
	  	...
	  	$query = $ci->db->get($table_name, $limit, $from);
	    return $query->result();
	}
}
...
if(!function_exists('db_get_row_data')) {
	...
	function db_get_row_data($table_name = null, $where = false, $limit=false, $from=false, $select_field=false) {
...
if(!function_exists('search_this_data')) {

	function search_this_data($table_name = null, $where = false, $limit=false, $from=false) {
...
if(!function_exists('is_allowed_group')) {

	function is_allowed_group($permission, Closure $func) {
		...
		if ($ci->aauth->is_group_allowed($permission)) {
		   	...
		   	foreach ($result as $row) {
		        ...
		        if ($row->Count > 0) {
		        	...
		        	if ($row->type != 'label') {
		        		...
		        		$ret .= '<div class="dd-handle dd3-handle dd-handles"><i class="fa '._ent($row->icon).' '._ent($row->icon_color).'"></i></div>';
...
if(!function_exists('display_menu_admin')) {
	...
	function display_menu_admin($parent, $level) {
		...
		// default query jika yang login itu developer 30 April 2018 Benkin

	    $result = $ci->db->query("SELECT a.id, a.label,a.icon_color, a.type, a.link,a.icon, a.developer_menu, Deriv1.Count FROM `menu` a  LEFT OUTER JOIN (SELECT parent, COUNT(*) AS Count FROM `menu` GROUP BY parent) Deriv1 ON a.id = Deriv1.parent WHERE a.menu_type_id = 1 AND a.parent=" . $parent." and active = 1  order by `sort` ASC")->result();



	    //jika yang login bukan developer, maka querynya ini 30 April 2018 Benkin

		// if ($_SESSION['dstat']!=NULL && $_SESSION['dstat']==0) {

		//     $result = $ci->db->query("SELECT a.id, a.label,a.icon_color, a.type, a.link,a.icon, a.developer_menu, Deriv1.Count FROM `menu` a  LEFT OUTER JOIN (SELECT parent, COUNT(*) AS Count FROM `menu` GROUP BY parent) Deriv1 ON a.id = Deriv1.parent WHERE a.developer_menu=0 AND a.menu_type_id = 1 AND a.parent=" . $parent." and active = 1  order by `sort` ASC")->result();
...
// clone data helper 20180823 by Benkin
if(!function_exists('clone_this_data')) {

	function clone_this_data($table_name = null, $where = false) {

		$ci =& get_instance();
		if (!$where) {
			return FALSE;
		}
	  	$get_data = $ci->db->get_where($table_name, $where);
	  	if (!$get_data) {
	  		return FALSE;
	  	}else{
	  		$primary_key = '';
	  		...
	  		$data->$primary_key = '';
	  		$insert_data = $ci->db->insert($table_name,$data);
		    return $get_data;
	  	}
	}
}

// check data helper 20180824 by Benkin
if(!function_exists('this_data_exist')) {

	function this_data_exist($table='', $where=false)
	{
		if (!$where) {
			return false;
		}else{
...
if(!function_exists('insert_this_data')) {

	function insert_this_data($table='', $data=array())
	{
		if (!is_array($data)) {
			return false;
		}else{
...
if(!function_exists('insert_this_data_last_id')) {

	function insert_this_data_last_id($table='', $data=array())
	{
		if (!is_array($data)) {
			return false;
		}else{
...
if(!function_exists('update_this_data')) {

	function update_this_data($table='', $where=false, $data=array())
	{
...
if(!function_exists('reset_this_data')) {

	function reset_this_data($table='', $data=array())
	{
		if ($data==null) {
			return false;
		}
...
// curl helper 20180921 by Benkin
if(!function_exists('custom_curl')) {

	function custom_curl($url='', $method='', $headers=array(), $body=array(), $upload=null)
	{	
		if($upload){
			$upload;
		}
