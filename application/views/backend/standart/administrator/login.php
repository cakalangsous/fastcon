...
<head>
  ...
  <link rel="icon" href="http://localhost:80/benkin/asset/logo/<?=get_option('site_favicon')?>">
...
<body class="hold-transition login-page" style="background-image: url('<?=BASE_ASSET?>logo/<?= get_option('login_bg'); ?>'); background-size: cover; background-repeat: no-repeat; overflow: hidden;">

<div class="login-box">

  <div class="login-logo">

    <a href="<?=site_url()?>" style="color:white; font-family: 'Helvetica Neue'"><b><?= cclang('login'); ?></b> <?= get_option('site_name'); ?></a>
  </div>
  ...
  <div class="login-box-body">
    ...
    <center> <a href="<?=site_url()?>"><img src="<?=BASE_ASSET?>logo/<?= get_option('site_logo'); ?>" alt="<?= get_option('site_name'); ?>" width="300"></a></center>
    <p class="login-box-msg" style="margin-top: 20px;"><?= cclang('sign_to_start_your_session'); ?></p>
    ...
    <?php endif; ?>
     ...
     <?= form_open('', [
      ...
      <div class="form-group has-feedback <?= form_error('username') ? 'has-error' :''; ?>">
        ...
        <input type="email" class="form-control" placeholder="Email" name="username" autofocus="autofocus">
